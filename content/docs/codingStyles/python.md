...
随着其他惯例的确定和过去的惯例因语言本身的变化而过时，本风格指南会随着时间的推移而改变。
...
但这可能会降低可读性：运算符往往分散在屏幕上的不同位置，并且每个运算符都远离其上一行的操作数。
...
为了解决这个可读性问题，数学家和他们的出版商遵循了相反的传统。
...
本指南确定与Knuth风格一致，代码应**在二进制运算符之前换行**。
...
Python代码应通过**UTF-8**进行编码，不需要代码中指明编码格式。
...
- 然而，在切片操作中，冒号的行为类似于二元运算符，因此在猫好两侧应留出相等的空格
...
尤其是当代码随着时间推移需要修改值、参数或导入项的列表时。
这种模式是将每个值或其他参数放在单独的一行上，并始终添加尾随逗号，
...
与代码相矛盾的注释比没有注释更糟糕。因此当代码发生变化时，切记优先确保注释是正确的！
...
在多句注释中，在句子结尾的句号后使用一个空格，除了最后一句之后。

确保你的注释清晰易懂，对其他使用您所编写的语言的人来说容易理解。

```python
# So this is a block comments, which can help
# the user understand what the code is doing
# or let other coders get on to the code more
# quickly. The principle of comments is to be
# easy to understand and more importantly,
# correct. Wrong comments are even better
# than having absolutely no comment.
```
...
还有一种使用短前缀将相关名称分组的风格,在Python中并不常用。
例如，`os.stat()`函数返回一个元组，
其中的元素传统上具有类似`st_mode`、`st_size`、`st_mtime`等名称。
（这样做是为了强调与POSIX系统调用结构的字段对应关系，这有助于熟悉该结构的程序员。）

此外，也可以使用以下前导或尾随下划线的特殊形式（这些通常可以与任何命名约定结合使用）：
...
- ``__double_leading_and_trailing_underscore__``：双下划线开头加结尾，
  ...
  例如，`init`、`import__`或`__file`。
  永远不要自行使用这种命名，只按照文档使用它们。
...
模块应具有简短的全小写名称。如果使用下划线可以提高可读性，那么允许在模块名称中使用下划线。
...
请注意，内置名称有单独的约定：大多数内置名称是单词（或两个单词连在一起），
只有在异常名称和内置常量中才使用大驼峰式命名规则。
...
就无法通过`Foo.__a`访问它（不过可以通过`Foo._Foo__a`强行访问）。
...
常量通常在模块级别上定义，并使用全大写字母，用下划线分隔单词。
...
我们这里不使用private这个术语，因为在Python中没有真正的私有属性
（除非做一些通常不必要的工作）。
...
- 在捕获操作系统错误时，推荐使用在Python 3.3中引入的显式异常层次结构，
